...
FLProgTimeToStringConverter::FLProgTimeToStringConverter(String string, RT_HW_STRUCT_UNIX_ID *time)
{
    _time = time;
    _inString = string;
    init();
...
uint8_t FLProgTimeToStringConverter::checkChar(char val, uint8_t index)
{
    ...
    {
        funcFillChar(_time->weekDay, FLPROG_RTC_ONE_SYMBOL_TYPE, index, FLPROG_RTC_FLASH_DAY_SYMBOL);
        return index + 1;
    }
    if (val == _charInput[1] /*	a	*/)
    {
        funcFillChar(((_time->hours < 12) ? 0 : 1) * 2, FLPROG_RTC_AM_PM_SYMBOL_TYPE, index, FLPROG_RTC_FLASH_AM_PM_SYMBOL);
        return index + 2;
    }
    if (val == _charInput[2] /*	A	*/)
    {
        funcFillChar((((_time->hours < 12) ? 0 : 1) + 2) * 2, FLPROG_RTC_AM_PM_SYMBOL_TYPE, index, FLPROG_RTC_FLASH_AM_PM_SYMBOL);
        return index += 2;
    }
    if (val == _charInput[3] /*	d	*/)
    {
        funcFillChar(_time->day, FLPROG_RTC_TWO_SYMBOL_TYPE, index, FLPROG_RTC_FLASH_DATE_SYMBOL);
        return index + 2;
    }
    if (val == _charInput[4] /*	h	*/)
    {
        funcFillChar(((_time->hours % 12) ? (_time->hours % 12) : 12), FLPROG_RTC_TWO_SYMBOL_TYPE, index, FLPROG_RTC_FLASH_HOUR_SYMBOL);
        return index + 2;
    }
    if (val == _charInput[5] /*	H	*/)
    {
        funcFillChar(_time->hours, FLPROG_RTC_TWO_SYMBOL_TYPE, index, FLPROG_RTC_FLASH_HOUR_SYMBOL);
        return index + 2;
    }
    if (val == _charInput[6] /*	i	*/)
    {
        funcFillChar(_time->minutes, FLPROG_RTC_TWO_SYMBOL_TYPE, index, FLPROG_RTC_FLASH_MINUTE_SYMBOL);
        return index + 2;
    }
    if (val == _charInput[7] /*	m	*/)
    {
        funcFillChar(_time->month, FLPROG_RTC_TWO_SYMBOL_TYPE, index, FLPROG_RTC_FLASH_MONTH_SYMBOL);
        return index + 2;
    }
    if (val == _charInput[8] /*	s	*/)
    {
        funcFillChar((_time->seconds), FLPROG_RTC_TWO_SYMBOL_TYPE, index, FLPROG_RTC_FLASH_SECOND_SYMBOL);
        return index + 2;
    }
    ...
    {
        funcFillChar((_time->month + 6) * 3, FLPROG_RTC_DAY_MONTH_SYMBOL_TYPE, index, FLPROG_RTC_FLASH_MONTH_SYMBOL);
        return index + 3;
    }
    if (val == _charInput[11] /*	D	*/)
    {
        funcFillChar(_time->weekDay * 3, FLPROG_RTC_DAY_MONTH_SYMBOL_TYPE, index, FLPROG_RTC_FLASH_DAY_SYMBOL);
        return index + 3;
    }
